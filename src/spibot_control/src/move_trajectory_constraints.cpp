...
std::array<float, 3> _BR_Backward_Trajectory(double passTime, int periodCnt)
{
    ...
    else if (passTime > (periodCnt + 1 / 2.0f) * swingPeriod && passTime <= (periodCnt + 2 / 3.0f) * swingPeriod)
    {
        ...
        sucker1_switch.data = false;
        BR_traj[0] = swingRaduis * cos(w1 * passTime) + x_bias;
        BR_traj[1] = y_bias - 0.05 * sin(w1 * passTime);
        BR_traj[2] = swingRaduis * sin(w1 * passTime) + z_bias;
    }
    else if (passTime > (periodCnt + 2 / 3.0f) * swingPeriod && passTime <= (periodCnt + 5 / 6.0f) * swingPeriod)
    {
...
std::array<float, 3> _BL_Backward_Trajectory(double passTime, int periodCnt)
{
    ...
    if (passTime > 1.0f * periodCnt * swingPeriod && passTime <= (periodCnt + 1 / 6.0f) * swingPeriod)
    {
        ...
        sucker4_switch.data = false;
        BL_traj[0] = -swingRaduis * cos(w1 * passTime) + x_bias;
        BL_traj[1] = y_bias + 0.05 * sin(w1 * passTime);
        BL_traj[2] = -swingRaduis * sin(w1 * passTime) + z_bias;
    }
    else if (passTime > (periodCnt + 1 / 6.0f) * swingPeriod && passTime <= (periodCnt + 1 / 3.0f) * swingPeriod)
    {
...
std::array<float, 3> _FL_Backward_Trajectory(double passTime, int periodCnt)
{
    ...
    else
    {
        ...
        sucker3_switch.data = false;
        FL_traj[0] = swingRaduis * cos(w1 * passTime) - x_bias;
        FL_traj[1] = y_bias - 0.05 * sin(w1 * passTime);
        FL_traj[2] = swingRaduis * sin(w1 * passTime) + z_bias;
    }
    rad = PosToTheta(FL_traj[0], FL_traj[1], FL_traj[2]);
    rad[2] = -rad[2];
...
std::array<float, 3> _FR_Forward_Trajectory(double passTime, int periodCnt)
{
    ...
    else if (passTime > (periodCnt + 1 / 2.0f) * swingPeriod && passTime <= (periodCnt + 2 / 3.0f) * swingPeriod)
    {
        ...
        FR_traj[1] = y_bias - 0.05 * sin(w1 * passTime);
...
std::array<float, 3> _BR_Rotation_Trajectory(double passTime, int periodCnt)
{
    ...
    std::array<float, 3> rad;
    BR_traj[1] = y_bias; // 沿直线前后摆动
    if (passTime > periodCnt * swingPeriod && passTime <= (periodCnt + 3 / 4.0f) * swingPeriod)
    {
        BR_traj[0] = ((-8.0 * swingRaduis) / (3.0f * swingPeriod)) * (passTime - 3.0 * swingPeriod / 8.0f - periodCnt * swingPeriod) - x_bias;
        BR_traj[2] = 0 + z_bias;
    }
    else
    {
        leg_is_moving.data = BR_leg;
        sucker1_switch.data = false;
        BR_traj[0] = swingRaduis * cos(w2 * passTime) - x_bias;
...
std::array<float, 3> _BL_Rotation_Trajectory(double passTime, int periodCnt)
{
    ...
    {
        BL_traj[0] = ((8.0f * swingRaduis) / (3.0f * swingPeriod)) * (passTime - 1.0 * swingPeriod / 8.0f - periodCnt * swingPeriod) - x_bias;
        BL_traj[2] = 0 + z_bias;
    }
    else if (passTime > (periodCnt + 3 / 4.0f) * swingPeriod && passTime <= (periodCnt + 1.0f) * swingPeriod)
    {
        BL_traj[0] = ((8.0 * swingRaduis) / (3.0f * swingPeriod)) * (passTime - 9.0 * swingPeriod / 8.0f - periodCnt * swingPeriod) - x_bias;
        BL_traj[2] = 0 + z_bias;
    }
    else
    {
        leg_is_moving.data = BL_leg;
        sucker4_switch.data = false;
        BL_traj[0] = swingRaduis * cos(w2 * passTime) - x_bias;
...
std::array<float, 3> _FL_Rotation_Trajectory(double passTime, int periodCnt)
{
    ...
    std::array<float, 3> rad;
    FL_traj[1] = y_bias;
    if (passTime > periodCnt * swingPeriod && passTime <= (periodCnt + 1 / 4.0) * swingPeriod)
    {
        FL_traj[0] = ((8.0 * swingRaduis) / (3.0 * swingPeriod)) * (passTime + 1.0 * swingPeriod / 8.0 - periodCnt * swingPeriod) + x_bias;
        FL_traj[2] = 0 + z_bias;
    }
    else if (passTime > (periodCnt + 1 / 2.0) * swingPeriod && passTime <= (periodCnt + 1.0) * swingPeriod)
    {
        FL_traj[0] = ((8.0 * swingRaduis) / (3.0 * swingPeriod)) * (passTime - 7.0 * swingPeriod / 8.0 - periodCnt * swingPeriod) + x_bias;
        FL_traj[2] = 0 + z_bias;
    }
    else
    {
        leg_is_moving.data = FL_leg;
        sucker3_switch.data = false;
        FL_traj[0] = -swingRaduis * cos(w2 * passTime) + x_bias;
        FL_traj[2] = swingRaduis * sin(w2 * passTime) + z_bias;
    }
    rad = PosToTheta(FL_traj[0], FL_traj[1], FL_traj[2]);
    rad[0] = -rad[0];
    rad[2] = -rad[2];
...
std::array<float, 3> _FR_Rotation_Trajectory(double passTime, int periodCnt)
{
    ...
    std::array<float, 3> rad;
    FR_traj[1] = y_bias;
    if (passTime > (periodCnt + 1 / 4.0) * swingPeriod && passTime <= (periodCnt + 1.0) * swingPeriod)
    {
        FR_traj[0] = ((-8.0 * swingRaduis) / (3.0f * swingPeriod)) * (passTime - 5.0 * swingPeriod / 8.0f - periodCnt * swingPeriod) + x_bias;
        FR_traj[2] = 0 + z_bias;
    }
    else
    {
        leg_is_moving.data = FR_leg;
        sucker2_switch.data = false;
        FR_traj[0] = -swingRaduis * cos(w2 * passTime) + x_bias;
